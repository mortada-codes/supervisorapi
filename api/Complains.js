...
const COMPLAINS_URL = (complainId) => (`http://192.168.50.123:5558/HelpdeskAPI/Api/Complains/${complainId}`)
...
const CHANGE_COMPLAIN_STATUS = () => (`http://192.168.50.123:5558/HelpdeskAPI/api/Mobile/ChangeComplainStatus`)
const Update_Complains_Transfered = (complainId , isTransfered) => (`http://192.168.50.123:5558/HelpdeskAPI/api/Complains/UpdateComplainIsTransfered?id=${complainId}&isTransfered=${isTransfered}`)
const Get_Complain_Statistics = (locationId , userId) => (`http://192.168.50.123:5558/HelpdeskAPI/api/Mobile/GetComplainStatisticsByLocations?locationId=${locationId}&userId=${userId}`)
const Get_Complains_Type = () => (`http://192.168.50.123:5558/HelpdeskAPI/api/ComplainType/GetAll`)
const Get_Complain_History = (complainId) => (`http://192.168.50.123:5558/HelpdeskAPI/api/Mobile/GetComplainHistoryByComplainId?ComplainId=${complainId}`)
const Get_Complain_Image_By_Id = () => (`http://192.168.50.123:5558/HelpdeskAPI/api/Mobile/GetMobileComplainImagesById`)
const Check_Complain_Version = (CurrentVersion) => (`http://192.168.50.123:5558/HelpdeskAPI/api/Complains/ChkMobileVersion?CurrentVersion=${CurrentVersion}`)
const Complain_Types_Dashboard = (ContractorId , userId , FilterContractorId , interval , datefrom , dateto , complainType , complainStatus) => (`http://192.168.50.123:5558/HelpdeskAPI/api/Complains/GetComplainTypesDashboard?ContractorID=${ContractorId}&userId=${userId}&FilterContractorID=${FilterContractorId}&interval=${interval}&datefrom=${datefrom}&dateto=${dateto}&complainType=${complainType}&complainStatus=${complainStatus}`)
const Update_Complain_Status = (id , statusId , userId , NotResolved , comment , ResonId) => (`http://192.168.50.123:5558/HelpdeskApi/api/Complains/UpdateComplianStatusAndaddComment?id=${id}&statusId=${statusId}&userId=${userId}&NotResolved=${NotResolved}&Comment=${comment}&ResonId=${ResonId}`)
const Complains_Which_Transfered_URL = (userId) => (`http://192.168.50.123:5558/HelpdeskAPI/api/Mobile/getComplainsWhichTransfered?userId=${userId}`)
const Get_Complain_Status = () => (`http://192.168.50.123:5558/HelpdeskAPI/api/Mobile/GetComplainsStatus`)
const Add_Complain_From_Web = () => (`http://192.168.50.123:5558/HelpdeskAPI/api/Complains/AddComplaintFromWeb`)
